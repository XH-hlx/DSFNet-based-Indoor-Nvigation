...
struct MappingData {
  ...
  std::vector<double> gradient_general_;

  // Semantic SSC data
  std::vector<uint8_t> semantic_labels_;      // Semantic labels per voxel
  std::vector<float> semantic_costs_;         // Traversal cost per voxel based on semantics
  ...
  // point cloud projected point
  pcl::PointCloud<pcl::PointXYZ> lidar_proj_points_;
  bool is_pcl_update;
...
public:
  ...
  inline void getOdom(Eigen::Vector3d &odom);
  inline int getOccupancy(Eigen::Vector3d pos);
  inline int getOccupancy(Eigen::Vector3i id);
  inline int getInflateOccupancy(Eigen::Vector3d pos);
  inline int getInflateOccupancy2d(Eigen::Vector2d pos);
  inline void getUpdatedBox(Eigen::Vector3i &b_min,Eigen::Vector3i &b_max);
  inline Eigen::Vector3i getMapSize();

  // Semantic query functions
  inline uint8_t getSemanticLabel(const Eigen::Vector3d& pos);
  inline uint8_t getSemanticLabel(const Eigen::Vector3i& id);
  inline float getSemanticCost(const Eigen::Vector3d& pos);
  inline float getSemanticCost(const Eigen::Vector3i& id);
  inline bool hasSemanticData() const;
...
private:
  ...
  void semanticCloudCallback(const sensor_msgs::PointCloud2ConstPtr& semantic_cloud);
...
inline bool GridMap::isKnownFree(const Eigen::Vector3i& id) {
  ...
  //return md_.occupancy_buffer_[adr] >= mp_.clamp_min_log_ && md_.occupancy_buffer_[adr] <= mp_.prob_hit_log_;
}
...
inline void GridMap::getOdom(Eigen::Vector3d &odom) {
    odom = md_.camera_pos_;
}
...
inline int GridMap::getInflateOccupancy2d(Eigen::Vector2d pos) {
    ...
    return int(md_.occupancy_buffer_inflate_2d_[toAddress2d(id(0),id(1))]);
}
...
inline void GridMap::getUpdatedBox(Eigen::Vector3i &b_min, Eigen::Vector3i &b_max) {
    b_min = md_.local_bound_min_;
    b_max = md_.local_bound_max_;
}

inline Eigen::Vector3i GridMap::getMapSize() {
...
inline uint8_t GridMap::getSemanticLabel(const Eigen::Vector3d& pos) {
  if (!isInMap(pos) || !md_.has_semantic_data_) return 0;
  Eigen::Vector3i id;
  posToIndex(pos, id);
  return md_.semantic_labels_[toAddress(id)];
}

inline uint8_t GridMap::getSemanticLabel(const Eigen::Vector3i& id) {
  if (!isInMap(id) || !md_.has_semantic_data_) return 0;
  return md_.semantic_labels_[toAddress(id)];
}

inline float GridMap::getSemanticCost(const Eigen::Vector3d& pos) {
  if (!isInMap(pos) || !md_.has_semantic_data_) return 1.0;
  Eigen::Vector3i id;
  posToIndex(pos, id);
  return md_.semantic_costs_[toAddress(id)];
}

inline float GridMap::getSemanticCost(const Eigen::Vector3i& id) {
  if (!isInMap(id) || !md_.has_semantic_data_) return 1.0;
  return md_.semantic_costs_[toAddress(id)];
}

inline bool GridMap::hasSemanticData() const {
  return md_.has_semantic_data_;
}
