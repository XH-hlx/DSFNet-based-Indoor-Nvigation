...
namespace ego_planner
{
    ...
    void BsplineOptimizer::setParam(ros::NodeHandle &nh)
    {
        ...
        nh.param("optimization/lambda_semantic", lambda_semantic_, 5.0);
        ...
        nh.param("optimization/use_semantic_cost", use_semantic_cost_, false);
    }
    ...
    {
        Eigen::Vector2d temp_pos2d;
        Eigen::Vector3d temp_pos3d;
        if (flag_first_init)
        {
        ...
        for (int i = order_; i <= i_end; ++i)
        {
            ...
            {
                temp_pos3d = a * init_points.col(i - 1) + (1 - a) * init_points.col(i);
                temp_pos2d<<temp_pos3d(0),temp_pos3d(1);
                occ = grid_map_->getInflateOccupancy2d(temp_pos2d);
                // cout << setprecision(5);
        ...
        for (size_t i = 0; i < segment_ids.size(); ++i)
        {
            ...
            Eigen::Vector2d in2d,out2d;
            in2d<<in(0),in(1);
            out2d<<out(0),out(1);
            if (a_star_->AstarSearch(/*(in-out).norm()/10+0.05*/ 0.1, in2d, out2d))
            {
            ...
            for (int j = segment_ids[i].first + 1; j < segment_ids[i].second; ++j)
            {
                ...
                if (got_intersection_id >= 0)
                {
                    ...
                    if (length > 1e-5)
                    {
                        ...
                        {
                            temp_pos3d = (a / length) * intersection_point + (1 - a / length) * cps_.points.col(j);
                            temp_pos2d<<temp_pos3d(0),temp_pos3d(1);
                            occ = grid_map_->getInflateOccupancy2d(temp_pos2d);
                                                   ...
                                                   Eigen::MatrixXd &gradient, int iter_num, double smoothness_cost)
    {
        ...
        /*** calculate distance cost and gradient with semantic awareness ***/
        for (auto i = order_; i < end_idx; ++i)
        {
            // Get semantic cost multiplier at this control point
            double semantic_multiplier = 1.0;
            if (use_semantic_cost_ && grid_map_->hasSemanticData())
            {
                Eigen::Vector3d pt_pos = cps_.points.col(i);
                float semantic_cost = grid_map_->getSemanticCost(pt_pos);
                // Scale semantic cost: higher semantic cost -> higher collision penalty
                semantic_multiplier = 1.0 + lambda_semantic_ * semantic_cost;
            }
            ...
            for (size_t j = 0; j < cps_.direction[i].size(); ++j)
            {
                ...
                {
                    cost += semantic_multiplier * pow(dist_err, 3);
                    gradient.col(i) += semantic_multiplier * (-3.0 * dist_err * dist_err * dist_grad);
                }
                else
                {
                    cost += semantic_multiplier * (a * dist_err * dist_err + b * dist_err + c);
                    gradient.col(i) += semantic_multiplier * (-(2.0 * a * dist_err + b) * dist_grad);
                }
            }
        }
    }
    ...
    bool BsplineOptimizer::check_collision_and_rebound(void)
    {
        ...
        {
            temp_pos3d = cps_.points.col(i);
            temp_pos2d<<temp_pos3d(0),temp_pos3d(1);
            bool occ = grid_map_->getInflateOccupancy2d(temp_pos2d);
    ...
    bool BsplineOptimizer::rebound_optimize()
    {
        ...
        do
        {
                ...
                result == lbfgs::LBFGS_STOP)
            {
                ...
                {
                    temp_pos3d = traj.evaluateDeBoorT(t);
                    temp_pos2d <<temp_pos3d(0),temp_pos3d(1);
                    flag_occ = grid_map_->getInflateOccupancy2d(temp_pos2d);
                    if (flag_occ)
                    {
    ...
    bool BsplineOptimizer::refine_optimize()
    {
        ...
        do
        {
            ...
            for (double t = tm; t < tmp * 2 / 3; t += t_step)
            {
                temp_pos3d = traj.evaluateDeBoorT(t);
                temp_pos2d <<temp_pos3d(0),temp_pos3d(1);
                if (grid_map_->getInflateOccupancy2d(temp_pos2d))
                {
