...
def get_score_2d_semantic(predict, target):
    _bs = predict.shape[0]  # batch size
    _C = predict.shape[1]  # _C = 12
    # ---- one-hot: _bs x _C x 480*640 -->  label: _bs x 480*640.
    predict = np.argmax(predict, axis=1)
    target = target.reshape(_bs, -1)    # (_bs, 307200)
    predict = predict.reshape(_bs, -1)  # (_bs, 307200), 480*640=307200
